...
losses.append(0.0 * 0.1e-7 * total_variation_loss(generated_image))

total_loss = sum(losses)

total_loss,_ = get_dist(photo, generated_image, {'mb_size' : 1, 'image_width' : 128})

grad = T.grad(total_loss, generated_image)

updates = {}

updates[generated_image] = generated_image - 1000.0 * grad

# Theano functions to evaluate loss and gradient
f_loss = theano.function([], total_loss)
f_grad = theano.function([], total_loss, updates = updates)
...
def deprocess(x):
    ...
    print "x min max deprocess", x.min(), x.max()
